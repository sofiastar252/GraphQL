...
const resolvers = {
      ...
      movies: (_, args) => {
        if (args.heroAge !== undefined) {
          return movies.filter(movie =>
            movie.heroes.some(hero => hero.age === args.heroAge)
          );
        }
        return movies;
      },
    ...
    addHeroToMovie: (_, { movieTitle, hero }) => {
      ...
      const existingHeroIndex = movies[movieIndex].heroes.findIndex(h =>
        h.name === hero.name
      );
    ...
    updateHeroInMovie: (_, { movieTitle, currentHeroName, newHero }) => {
      ...
      const heroIndex = movies[movieIndex].heroes.findIndex(h =>
        h.name === currentHeroName
      );
      if (heroIndex === -1) {
